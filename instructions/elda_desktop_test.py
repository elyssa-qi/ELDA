# elda_desktop_test.py - Desktop AI Assistant with Step-by-Step Modal (Test Version)
...
class EldaDesktopTest:
    def __init__(self, root):
        self.root = root
        self.root.title("Elda - AI Assistant (Test Mode)")
        ...
        subtitle = tk.Label(title_frame, text="(Test Mode)",
                           font=('Segoe UI', 12),
        ...
        self.speak_button = tk.Checkbutton(
            ...
            bd=0,
            highlightthickness=0,
            state=tk.DISABLED  # Disabled in test mode
        )
        ...
        self.add_message("Note: This is test mode. Voice input and AI responses are disabled.", 'system')
    ...
    def process_question(self, question):
        """Process question (runs in background thread)"""
        try:
            ...
            if self.is_step_by_step_request(question):
                # Generate mock steps
                steps_data = self.generate_mock_steps(question)
    ...
    def generate_mock_steps(self, question):
        """Generate mock steps for testing"""
        # Extract task from question
        task = question.replace("how to", "").replace("how do i", "").strip()
        
        # Mock steps based on common tasks
    ...
    def start_voice_input(self):
        """Start listening for voice input (disabled in test mode)"""
        self.add_message("Voice input is disabled in test mode.", 'system')

def main():
    root = tk.Tk()
    app = EldaDesktopTest(root)
